...
encWire.i2cSendByte(ENC_REG_STATUS);
...
response[0] = encWire.i2cReceiveByte();
...
encWire.i2cSendByte(ENC_REG_ANG_FIL1);
...
response[1] = encWire.i2cReceiveByte();
...
for(;;) {
	...
	uint8_t brightness = angle / 16;
	pixel.setPixelColor(0,brightness,brightness,brightness);
	//pixel.setPixelColor(0,wheel(angle/16));
	pixel.show();
	HAL_Delay(10); //delay
...
pixel.setPixelColor(0,127,127,0);
pixel.show();
HAL_Delay(500); //delay
...
pixel.setPixelColor(0,0,127,127);
pixel.show();
HAL_Delay(500); //delay
...
pixel.setPixelColor(0,127,0,127);
pixel.show();
HAL_Delay(500); //delay
...
for(;;) {
	...
	pixel.setPixelColor(0,getRainbow());
	pixel.show();
	HAL_Delay(1); //delay
